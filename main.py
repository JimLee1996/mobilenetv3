...
def train_one_epoch(
    ...
    train_logger = utils.FileLogger(
        'train.log', ['epoch', 'lr', 'loss', 'acc1', 'acc5']
    )
    train_logger.log(
        {
            'epoch': epoch,
...
def evaluate(model, criterion, data_loader, device, epoch):
    ...
    if epoch != None:
        val_logger = utils.FileLogger(
            'val.log', ['epoch', 'loss', 'acc1', 'acc5']
        )
        val_logger.log(
            {
                'epoch': epoch,
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_one_epoch(
            model, criterion, optimizer, data_loader, device, epoch,
            args.print_freq
        )
        lr_scheduler.step()
...
def parse_args():
    ...
    parser.add_argument(
        '--output-dir', default='pth', help='path where to save model'
    )
